...
$nombreArc=$rucemisor.'-'.$tipoComp.'-'.$serieComp.'-'.$numeroComp;

//Descargamos el Archivo Xml que contiene el ZIP del CDR
$archivo = fopen('C'.$nombreArc.'.xml','w+');
fputs($archivo,$result);
fclose($archivo);

/*LEEMOS EL ARCHIVO XML*/
$xml = simplexml_load_file('C'.$nombreArc.'.xml'); 
foreach ($xml->xpath('//content') as $response){ }

/*AQUI DESCARGAMOS EL ARCHIVO CDR(CONSTANCIA DE RECEPCIÓN)*/
$cdr=base64_decode($response);

$archivo = fopen('R'.$nombreArc.'.zip','w+');
fputs($archivo,$cdr);
fclose($archivo);


//Eliminamos el Archivo
if (file_exists('C'.$nombreArc.'xml')) {
   unlink('C'.$nombreArc.'xml');
}
